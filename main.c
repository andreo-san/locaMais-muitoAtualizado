...
printf("11. Backup\n");
printf("12. Restore\n");
printf("13. cadastro de locação\n");
printf("14. Baixa na Locação\n");
printf("15. Pesquisar locação por cliente:\n");
printf("0. Sair\n");
printf("Digite sua opcao: ");
scanf("%d", &opcao);
if ((opcao < 0) || (opcao > 17))
printf ("Opcao Digitada Incorreta!\n");
} while ((opcao < 0) || (opcao > 17));
...
void cadastrarLocacao()
{
    ...
    printf("Digite o rg do cliente:/n");
    scanf("%i", &codCliente);
    ...
    printf("Digite o código do carro a ser alugado:/n");
    scanf("%i", &codCarro);

    printf("Deseja fazer seguro?(Digite 1 para sim e 0 para não):/n");
    scanf("%i", &seguro);
...
while (fread (&clientes, sizeof(clientes), 1, arq))
printf("Cod %u --- Descricao: %-8s --- Valor R$ %4.2f --- Deletado?(%c) --- Pontos (%i)\n",clientes.rgCliente, clientes.nome, clientes.valor, clientes.deletado, clientes.pontos);
